...
function autocomplete(input, latInput, lngInput) {
  if (!input) return; //skip this function with no input
...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    var markers = places.map(function (places) {
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios.get('/api/search?q=' + this.value).then(function (res) {
      ...
      //nothing came back
      searchResults.innerHTML = dompuify.sanitize('<div class=\'search__result\'>No results for ' + _this.value + ' found!</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
